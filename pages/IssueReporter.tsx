...
export const IssueReporter: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const prompt = `נתח תקלה. נושא: ${summary} תיאור: ${description}. החזר JSON עם category ו-priority.`;
  ...
  if (!isAuthenticated) { /* ... קוד מסך הלוגין נשאר זהה ... */ return (
    <div className="flex-1 min-h-screen bg-slate-50 flex justify-center items-center p-6 w-full">
      <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl border border-slate-100 text-right">
  ...
  return (
    <div className="h-screen w-full bg-slate-100 flex justify-center items-center p-4">
      <div className="absolute top-6 left-6 flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-slate-200">
         <User size={16} /> <span className="text-sm font-medium">{userName}</span>

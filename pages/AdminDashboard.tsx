                  ...
                  <form onSubmit={handleLogin} className="space-y-4">
                      <div>
                          ...
                          <div className="relative">
                              ...
                              <input type="text" value={adminName} onChange={(e) => setAdminName(e.target.value)} className="w-full pr-10 pl-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="שם המנהל/ת" required />
                          </div>
                      </div>
                      <div>
                          <label className="block text-sm font-medium text-slate-700 mb-1">סיסמה</label>
                          <input type="password" value={adminPassword} onChange={(e) => { setAdminPassword(e.target.value); setAuthError(false); }} className={`w-full px-3 py-2.5 bg-slate-50 border rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-colors ${authError ? 'border-red-500 bg-red-50' : 'border-slate-200'}`} placeholder="הזן סיסמה..." required />
                          {authError && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={12} /> סיסמה שגויה. נסה שנית.</p>}
                      </div>
                      <button type="submit" className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-500/30 transition-all flex items-center justify-center gap-2 mt-4">כניסה למערכת <ArrowLeft size={20} /></button>
                  </form>
              </div>
          </div>
      );
        ...
        <div className="max-w-7xl mx-auto w-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1">
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-indigo-100 sticky top-0">
                  ...
                  <div onClick={() => !isReadingFile && fileInputRef.current?.click()} className={`mb-4 border-2 border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition-all group ${isReadingFile ? 'opacity-50 cursor-wait' : ''}`}>
                    ...
                    {isReadingFile ? <Loader2 size={32} className="text-indigo-600 animate-spin mb-2" /> : <div className="bg-indigo-100 p-3 rounded-full mb-3 group-hover:bg-indigo-200 transition-colors"><FileUp size={24} className="text-indigo-600" /></div>}
                    <p className="text-sm font-medium text-slate-600 text-center">{isReadingFile ? 'קורא קובץ...' : 'לחץ להעלאת קובץ'}</p>
                    <p className="text-xs text-slate-400 mt-1 text-center">TXT, JSON, CSV, PDF</p>
                    {!isReadingFile && uploadedFileName && <div className="mt-3 flex items-center gap-2 text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full"><CheckCircle size={12} /> {uploadedFileName}</div>}
                  </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                 <h2 className="text-lg font-bold text-slate-700 mb-2 flex items-center gap-2"><FileText size={20} className="text-slate-500" /> פריטי ידע קיימים ({knowledgeItems.length})</h2>
                {knowledgeItems.length === 0 ? (
                    <div className="text-center py-10 bg-white rounded-2xl border border-dashed border-slate-300 text-slate-400">עדיין אין פריטי ידע. הוסף את הראשון!</div>
                ) : (
                    ...
                    <div key={item.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow group relative">
                        <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-2">
                                ...
                                {item.sourceType === 'file' && <span className="px-2 py-0.5 bg-blue-50 text-blue-600 text-xs rounded-full border border-blue-100 flex items-center gap-1"><FileJson size={10} /> קובץ</span>}
                            </div>
            ...
            <div className="max-w-7xl mx-auto w-full">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => { loadData(); generateTrends(queries); }} className="p-2 bg-white border border-slate-200 rounded-full hover:bg-slate-50 text-slate-500 hover:text-indigo-600 transition-colors"><RefreshCw size={20} className={trendsLoading ? "animate-spin" : ""} /></button>
                    </div>
                </div>
                ...
                <div className="mb-8">
                    ...
                    {trendsLoading ? (
                        ...
                        <div className="bg-slate-50 rounded-xl p-6 text-center text-slate-400 text-sm">לא נמצאו מספיק נתונים לניתוח מגמות עדיין.</div>
                    )}
                </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex flex-wrap gap-4 items-center">
                    ...
                    <div className="relative">
                        ...
                        <input type="text" value={queryFilterUser} onChange={(e) => setQueryFilterUser(e.target.value)} placeholder="חפש לפי שם עובד..." className="p-2 pr-9 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500 w-48" />
                    </div>
                    <input type="date" value={queryFilterDate} onChange={(e) => setQueryFilterDate(e.target.value)} className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
                    <div className="w-px h-8 bg-slate-200 mx-2 hidden md:block"></div>
                    <div className="flex items-center gap-2 text-slate-500 font-medium"><ArrowUpDown size={18} /> מיון:</div>
                    <select value={querySortOption} onChange={(e) => setQuerySortOption(e.target.value as any)} className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
                    <div className="p-4 border-b border-slate-100 bg-slate-50/50"><h3 className="font-bold text-slate-700 flex items-center gap-2"><MessageSquare size={18} /> יומן שאלות מלא</h3></div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-right border-collapse">
                            <thead className="bg-slate-50 text-slate-700 text-sm font-bold border-b border-slate-200">
                                <tr><th className="p-4 whitespace-nowrap w-32">תאריך</th><th className="p-4 whitespace-nowrap w-32">שם עובד</th><th className="p-4 whitespace-nowrap w-32">מחלקה</th><th className="p-4 min-w-[200px]">מהות השאלה</th><th className="p-4 min-w-[200px]">תשובת המערכת</th><th className="p-4 whitespace-nowrap w-24">סטטוס</th></tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 text-sm">
                                {filteredQueries.map(q => (
                                    <tr key={q.id} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="p-4 text-slate-500 align-top">{new Date(q.timestamp).toLocaleDateString('he-IL')} <div className="text-xs opacity-60">{new Date(q.timestamp).toLocaleTimeString('he-IL', {hour: '2-digit', minute:'2-digit'})}</div></td>
                                        <td className="p-4 font-medium text-slate-800 align-top">{q.username || 'אורח'}</td>
                                        <td className="p-4 text-slate-600 align-top">{q.department || '-'}</td>
                                        <td className="p-4 text-slate-700 font-medium align-top">{q.question}</td>
                                        <td className="p-4 text-slate-500 text-xs leading-relaxed align-top"><div className="line-clamp-2 hover:line-clamp-none transition-all cursor-default">{q.answer}</div></td>
                                        <td className="p-4 align-top">{q.isAnswered ? <span className="flex items-center gap-1 text-green-600 bg-green-50 px-2 py-1 rounded-full text-xs font-semibold w-fit"><CheckCircle size={12} /> נענתה</span> : <span className="text-red-500 bg-red-50 px-2 py-1 rounded-full text-xs">שגיאה</span>}</td>
                                    </tr>
                                ))}
                                {filteredQueries.length === 0 && <tr><td colSpan={6} className="p-12 text-center text-slate-400">לא נמצאו שאלות התואמות את הסינון</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto w-full">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={loadData} className="p-2 bg-white border border-slate-200 rounded-full hover:bg-slate-50 text-slate-500 hover:text-indigo-600 transition-colors"><RefreshCw size={20} /></button>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex flex-wrap gap-4 items-center">
                    ...
                    <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="all">כל הקטגוריות</option>
                        {Object.values(IssueCategory).map(cat => <option key={cat} value={cat}>{cat}</option>)}
                    </select>
                    <select value={filterPriority} onChange={(e) => setFilterPriority(e.target.value)} className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="all">כל הדחיפויות</option>
                        {Object.values(IssuePriority).map(prio => <option key={prio} value={prio}>{prio}</option>)}
                    </select>
                    <div className="relative">
                        <Search size={16} className="absolute top-2.5 right-3 text-slate-400" />
                        <input type="text" value={filterUser} onChange={(e) => setFilterUser(e.target.value)} placeholder="חפש לפי שם משתמש..." className="p-2 pr-9 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500 w-48" />
                    </div>
                    <input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
                    <div className="w-px h-8 bg-slate-200 mx-2 hidden md:block"></div>
                    <div className="flex items-center gap-2 text-slate-500 font-medium"><ArrowUpDown size={18} /> מיון:</div>
                    <select value={sortOption} onChange={(e) => setSortOption(e.target.value as any)} className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="newest">מהחדש לישן</option>
                        <option value="oldest">מהישן לחדש</option>
                        <option value="open_first">סטטוס פתוח קודם</option>
                    </select>
                    <div className="mr-auto text-xs text-slate-400 font-medium">נמצאו {filteredIssues.length} רשומות</div>
                </div>
                            ...
                            <tbody className="divide-y divide-slate-100 text-sm">
                                {filteredIssues.map(issue => (
                                    <tr key={issue.id} className="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="p-4">
                                            {editingIssue === issue.id ? (
                                                 <select value={editPriority} onChange={(e) => setEditPriority(e.target.value as any)} className="p-1.5 border border-slate-300 rounded-lg text-xs">
                                                    {Object.values(IssuePriority).map(prio => <option key={prio} value={prio}>{prio}</option>)}
                                                 </select>
                                            ) : (
                                        ...
                                        <td className="p-4">
                                            {editingIssue === issue.id ? (
                                                ...
                                                <span className={`px-2.5 py-1 text-xs font-semibold rounded-full border ${issue.status === 'open' ? 'bg-red-50 text-red-600 border-red-100' : issue.status === 'in_progress' ? 'bg-orange-50 text-orange-600 border-orange-100' : issue.status === 'transferred' ? 'bg-purple-50 text-purple-600 border-purple-100' : 'bg-green-50 text-green-600 border-green-100'}`}>
                                                    {issue.status === 'open' ? 'פתוח' : issue.status === 'in_progress' ? 'בטיפול' : issue.status === 'transferred' ? 'הועבר לטיפול' : 'סגור'}
                                                </span>
                                            )}
                                        </td>
                                        <td className="p-4">
                                        ...
                                        <td className="p-4">
                                            {editingIssue === issue.id ? (
                                                <button onClick={() => saveEdit(issue)} className="flex items-center gap-1 bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-xs hover:bg-indigo-700 shadow-sm"><Save size={14} /> שמור</button>
                                            ) : (
                                                <button onClick={() => startEditing(issue)} className="text-indigo-600 hover:text-indigo-800 text-xs font-medium border border-indigo-200 hover:border-indigo-400 px-3 py-1.5 rounded-lg transition-all">עדכן</button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
    ...
    <div className={screenWrapperClass}>
      <div className="max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-center gap-2 mb-8 bg-white p-1 rounded-xl w-fit border border-slate-200 shadow-sm flex-wrap">
              ...
              <button onClick={() => setDashboardTab('users')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${dashboardTab === 'users' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}><Users size={16} /> טיפול בעובדים (User Care)</button>
          </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 w-full">
                        ...
                        <div className="flex flex-wrap gap-3 justify-center mt-2">
                            {priorityData.map((entry, index) => (
                                <div key={entry.name} className="flex items-center gap-1 text-xs text-slate-500"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[index % COLORS.length]}}></div>{entry.name}</div>
                            ))}
                        </div>
                    </div>
                </div>
          ...
          {dashboardTab === 'tasks' && (
              <div className="animate-fadeIn w-full">
                  <div className="flex items-center gap-2 mb-4 text-slate-700 font-bold text-lg"><Bell size={20} className="text-red-500" /> משימות פתוחות לטיפול ({openTasksCount})</div>
                              ...
                              <div className="flex justify-between items-start">
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${issue.priority === IssuePriority.CRITICAL ? 'bg-red-100 text-red-600' : issue.priority === IssuePriority.HIGH ? 'bg-orange-100 text-orange-600' : issue.priority === IssuePriority.MEDIUM ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-600'}`}>{issue.priority}</span>
                                            {issue.attachments && issue.attachments.length > 0 && <span className="flex items-center gap-1 px-2 py-0.5 text-xs font-semibold rounded-full bg-slate-100 text-slate-600"><Paperclip size={10} />{issue.attachments.length} קבצים</span>}
                                        </div>
                                        <p className="text-slate-600 text-sm mb-3">{issue.description}</p>
                                        <div className="flex items-center gap-4 text-xs text-slate-400"><span>{new Date(issue.createdAt).toLocaleDateString('he-IL')}</span><span>•</span><span>{issue.username} ({issue.userRole})</span></div>
                                    </div>
                                    {editingIssue !== issue.id && <button onClick={() => startEditing(issue)} className="text-indigo-600 hover:bg-indigo-50 px-4 py-2 rounded-lg text-sm font-medium transition-colors">טפל בתקלה</button>}
                              </div>
                                  ...
                                  <div className="mt-4 pt-4 border-t border-slate-100 animate-fadeIn">
                                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
                                            <div className="md:col-span-1">
                                                ...
                                                <select value={editStatus} onChange={(e) => setEditStatus(e.target.value as any)} className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none">
                          ...
                          <div className="space-y-4">
                              {userStats.slice(0, 5).map((user, idx) => (
                                  <div key={idx} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                                      ...
                                      <div className="flex-1"><div className="font-bold text-slate-800 text-sm">{user.name}</div><div className="text-xs text-slate-500">{user.department}</div></div>
                                      <div className="text-right"><div className="font-bold text-indigo-600 text-sm">{user.total}</div><div className="text-[10px] text-slate-400">פניות</div></div>
                                  </div>
                              ))}
                               {userStats.length === 0 && <div className="text-center text-slate-400 py-8 text-sm">אין מספיק נתונים</div>}
                          </div>
                      </div>
                  </div>
              </div>

...
const loadData = async () => {
  ...
  const generateInsight = async (currentIssues: Issue[]) => {
      // Taking last 10 issues for context to avoid token limits in this demo
...
// --- Log to Google Sheets (Knowledge_Base) ---
  ...
  const handleDeleteKnowledge = (id: string) => {
      ...
      // In real app: StorageService.deleteKnowledgeItem(id);
  };
  ...
  // --- Processed Data for Views (Moved up for Export access) ---
  ...
  const exportToExcel = () => {
      ...
      // Use filteredIssues for WYSIWYG export
      const rows = filteredIssues.map(i => [
          ...
          i.category,
          i.priority,
          `"${i.summary.replace(/"/g, '""')}"`, // Escape quotes
  ...
  const exportQueriesToExcel = () => {
      ...
      // Use filteredQueries for WYSIWYG export
